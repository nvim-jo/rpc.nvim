...
--                                              --
--   Discord Rich RPC plugin for Neovim.   --
--------------------------------------------------

local RPC = {}
RPC.is_authorized = false
RPC.is_authorizing = false
RPC.is_connected = false
RPC.is_connecting = false
RPC.last_activity = {}
RPC.peers = {}
RPC.socket = vim.v.servername
RPC.workspace = nil
RPC.workspaces = {}
...
local file_explorers = require("rpc.file_explorers")
local default_file_assets = require("rpc.file_assets")
local plugin_managers = require("rpc.plugin_managers")
local Discord = require("rpc.discord")
...
function RPC:setup(...)
    ...
    self:set_option("client_id", "1151042952404148265")
    self:set_option("debounce_timeout", 10)
    self:set_option("main_image", "neovim")
    self:set_option("neovim_image_text", "True Text Editor")
    ...
    vim.api.nvim_set_var("rpc_auto_update", options.auto_update)

    -- Set autocommands
    vim.fn["rpc#SetAutoCmds"]()

    self.log:info("Completed plugin setup")

    -- Set global variable to indicate plugin has been set up
    vim.api.nvim_set_var("rpc_has_setup", 1)
...
function RPC.get_os_name(uname)
...
function RPC.coalesce_option(value)
...
function RPC:set_option(option, default, validate)
    default = self.coalesce_option(default)
    validate = validate == nil and true or validate

    local g_variable = string.format("rpc_%s", option)
...
function RPC:check_dup_options(option)
    local g_variable = string.format("rpc_%s", option)
...
function RPC:check_discord_socket(path)
...
function RPC:call_remote_nvim_instance(socket, command)
...
-- Call a RPC method on a remote instance with a given list of arguments
function RPC:call_remote_method(socket, name, args)
    local command_fmt = "lua package.loaded.rpc:%s(%s)"
...
function RPC:authorize(on_done)
...
function RPC:get_discord_socket_path()
...
function RPC.get_current_buffer()
    local current_buffer = vim.api.nvim_get_current_buf()
    return vim.api.nvim_buf_get_name(current_buffer)
end

-- Gets the current project name
function RPC:get_project_name(file_path)
...
function RPC:get_status_text(filename)
...
function RPC:get_nvim_socket_paths(on_done)
...
function RPC.discord_event(on_ready)
...
function RPC:check_blacklist(buffer, parent_dirpath, project_dirpath)
...
function RPC:get_buttons(buffer, parent_dirpath)
...
-- Update Rich RPC for the provided vim buffer
function RPC:update_for_buffer(buffer, should_debounce)
...
-- Update Rich RPC for the current or provided vim buffer for an authorized connection
RPC.update = RPC.discord_event(function(self, buffer, should_debounce)
    -- Default update to not debounce by default
    if should_debounce == nil then should_debounce = false end

    -- Debounce Rich RPC updates (default to 10 seconds):
...
--------------------------------------------------
-- RPC peer-to-peer API
--------------------------------------------------

-- Register some remote peer
function RPC:register_peer(id, socket)
...
function RPC:unregister_peer(id, peer)
...
function RPC:unregister_peer_and_set_activity(id, peer)
    self:unregister_peer(id, peer)
    self:update()
end

-- Register a remote peer and sync its data
function RPC:register_and_sync_peer(id, socket)
...
function RPC:sync_self_activity()
    local self_as_peer = {
        socket = self.socket,
        workspace = self.workspace,
    }
...
function RPC:sync_peer_activity(data)
...
function RPC:handle_focus_gained()
...
function RPC:handle_text_changed()
...
function RPC:handle_vim_leave_pre()
